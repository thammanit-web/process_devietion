...
export default function FirstPage() {
  ...
  const allowedNames = ["Pongjuta@thainitrate.com", "Weerapan@thainitrate.com", "Jaroonrote@thainitrate.com", "Thammanit@thainitrate.com","Sysadmin@thainitrate.com"];
  ...
  const isAllowed = allowedTechnical.includes(userDetails?.department) || allowedNames.includes(userDetails?.mail);
  const isAllowedManager = allowedManager.includes(userDetails?.department) || allowedNames.includes(userDetails?.mail);;
  const isAllowedMaintenance = allowedMaintenance.includes(userDetails?.department) || allowedNames.includes(userDetails?.mail);
  const isAllowedCreate = allowedCreate.includes(userDetails?.department) || allowedNames.includes(userDetails?.mail) || userDetails?.mail === "Worrawut@thainitrate.com";

  return (
    <div className="overflow-x-auto mt-16 py-4">
      <div className='grid justify-center items-center'>
        <div className="flex justify-center gap-10 mb-6" >
          <a href="/production" className={`md:text-lg sm:text-sm py-4 text-white h-14 w-72 flex items-center rounded-2xl 
              ${isAllowedCreate ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 cursor-not-allowed opacity-50'}`}
            style={{ pointerEvents: isAllowedCreate ? 'auto' : 'none' }}>
        ...
        <div className="flex justify-center gap-10" >
          ...
          <div className='grid justify-center gap-8'>
            <a href="/history" className="hover:bg-green-600 md:text-lg sm:text-sm gap-2 py-4 text-white h-14 w-72 flex  rounded-2xl bg-green-500 shadow-md drop-shadow-md border shadow-gray-200 hover:shadow-white">
            ...
            <a href="/manager_page" className={`md:text-lg sm:text-sm py-4 text-white h-14 w-72 flex items-center rounded-2xl 
              ${isAllowedManager ? 'bg-blue-500 hover:bg-blue-600' : 'bg-green-500 cursor-not-allowed opacity-50'}`}
